...
async def ingest_rows(user_id: UUID, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    """
    inserted, skipped = await upsert_finance_rows(user_id, rows)

    from shared.db import fetch_uncategorized_ids
    uncat_all = await fetch_uncategorized_ids(user_id)
...
async def categorize_new(user_id: UUID, uncategorized_ids: list[str]) -> None:
    ...
    desc_by_id = await fetch_descriptions_for(user_id, uncategorized_ids)
    if not desc_by_id:
        return
    ...
    if new_mappings:
        await upsert_category_cache(user_id, new_mappings)
        row_updates = {
            tid: new_mappings[k]
            for tid, k in key_by_id.items() if k in new_mappings
        }
        if row_updates:
            await set_transaction_categories(user_id, row_updates)
...
def build_upload_summary(
    *,
    ...
    """Build a human-readable summary string from ingest + query outputs."""
    parts: list[str] = []
