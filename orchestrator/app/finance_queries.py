...
async def income_for_month(user_id: UUID, month_str: str) -> dict[str, Decimal]:
    """Sum of IN amounts for the calendar month, per currency.

    `month_str` is `YYYY-MM` (e.g. "2026-04").
    """
    rows = await finance_rows_for_month(user_id, month_str)
...
async def current_balance(user_id: UUID) -> dict[str, Decimal]:
    """Signed running balance per currency: sum(IN) − sum(OUT)."""
    rows = await finance_rows_all(user_id)
    out: dict[str, Decimal] = {}
    for r in rows:
        cur = r["currency"]
        amt = r["amount"]
...
async def runway(user_id: UUID, avg_window_days: int = 30) -> dict[str, dict[str, Any]]:
    ...
    window = await finance_rows_in_window(user_id, avg_window_days)
