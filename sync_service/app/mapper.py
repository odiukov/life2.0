...
def map_hrv(date_str: str, raw: dict) -> dict | None:
    """Map Garmin get_hrv_data() response to a health_logs row."""
    summary = (raw or {}).get("hrvSummary", {})
    last_night = summary.get("lastNightAvg") or summary.get("lastNight")
    if last_night is None:
        return None
    from datetime import datetime, timezone
    recorded_at = datetime.fromisoformat(f"{date_str}T06:00:00").replace(tzinfo=timezone.utc)
    return {
        "agent": "sleep",
        "type": "hrv_status",
        "data": {
            "hrv_rmssd": int(last_night),
...
def map_activity(raw: dict) -> dict | None:
    ...
    # startTimeGMT is already UTC; startTimeLocal is device-local (misleadingly named)
