...
class SleepAgentExecutor(AgentExecutor):
    async def execute(self, ctx: RequestContext, event_queue: EventQueue) -> None:  # noqa: D401
        ...
        try:
            ...
            # The "most recent" sleep record reflects last-night's sleep, which
            # ended this morning but was driven by YESTERDAY's intake/training.
            # Scope peer consults to yesterday so nutrition/workout summarize
            # the day that actually influenced the sleep — not today's data.
            yesterday = (datetime.now(timezone.utc).date() - timedelta(days=1)).isoformat()
            # Depth-1 cap: when this sleep call was issued by another agent
            # via call_peer(), do not consult any further peers — return only
            # sleep-domain analysis.
