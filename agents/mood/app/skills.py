...
SKILLS: list[AgentSkill] = [
    ...
    AgentSkill(
        ...
        description=(
            ...
            "mood entry with source_skill='coach_session'. Invoked by the telegram "
            "coach loop on session finalization."
        ),
        tags=["mood", "coach"],
...
async def _prompt_analyze(message: str, params: dict) -> str:
    return await build_mood_prompt("analyze_mood", {**params, "message": message})


async def _prompt_recommendations(message: str, params: dict) -> str:
    return await build_mood_prompt("get_recommendations", {**params, "message": message})
...
SKILL_PROMPTS: dict[str, PromptFn] = {
    "log_mood": _prompt_log_mood,
    "analyze_mood": _prompt_analyze,
    "get_recommendations": _prompt_recommendations,
    "coach_session": _prompt_coach_session,
}
