...
async def test_log_habit_check_resolves_name_and_inserts_log():
    ...
    async def fake_insert_log(user_id, agent, type_, data, source="manual"):
        captured["user_id"] = user_id
        captured["data"] = data
        captured["type"] = type_
        captured["agent"] = agent

    async def fake_find(user_id, name):
        return {
...
async def test_archive_habit_happy_path():
    ...
    async def fake_archive(user_id, habit_id):
