...
def _fresh_servers(monkeypatch, env: dict[str, str]) -> dict:
    ...
    for key in ("MCP_GOOGLE_CALENDAR_URL", "MCP_GOOGLE_CALENDAR_TRANSPORT",
...
def test_ha_branch_when_both_env_present(monkeypatch):
    servers = _fresh_servers(monkeypatch, {
        "HA_BASE_URL": "http://192.168.1.85:8123",
        "HA_TOKEN": "tok-abc",
    })
    assert "home-assistant" in servers
    ha = servers["home-assistant"]
    assert ha["url"] == "http://192.168.1.85:8123/mcp_server/sse"
...
def test_ha_base_url_trailing_slash_stripped(monkeypatch):
    ...
    assert servers["home-assistant"]["url"] == "http://192.168.1.85:8123/mcp_server/sse"


def test_calendar_and_ha_coexist(monkeypatch):
    servers = _fresh_servers(monkeypatch, {
        "MCP_GOOGLE_CALENDAR_URL": "http://calendar-mcp:3000",
        "HA_BASE_URL": "http://192.168.1.85:8123",
        "HA_TOKEN": "tok-abc",
    })
