...
async def _cleanup():
    ...
    try:
        await conn.execute(
            "DELETE FROM habits WHERE user_id = $1 AND name LIKE 'reg-test-%'",
            TEST_USER_ID,
        )
    finally:
        await conn.close()
...
async def test_upsert_and_find_by_name():
    ...
    found = await registry.find_by_name(TEST_USER_ID, "Reg-Test-Meditation")
...
async def test_find_by_name_returns_none_for_unknown():
    if not _has_db():
        pytest.skip("POSTGRES_DSN not set")
    from agents.habits.app import registry
    assert await registry.find_by_name(TEST_USER_ID, "reg-test-nonexistent") is None
...
async def test_archive_hides_from_list_but_allows_recreate():
    ...
    active = await registry.list_active(TEST_USER_ID)
    assert any(h["id"] == first_id for h in active)

    assert await registry.archive(TEST_USER_ID, first_id) is True
    active_after = await registry.list_active(TEST_USER_ID)
