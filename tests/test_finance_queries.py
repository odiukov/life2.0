...
# Fixed test user UUID to scope all finance query test rows
_TEST_USER_ID = UUID("11111111-1111-2222-aaaa-aaaaaaaaaaaa")
...
async def _cleanup():
    ...
    try:
        ...
        await conn.execute(
            ...
            _TEST_USER_ID, "test-finance-queries",
        )
        await conn.execute(
            "DELETE FROM finance_transactions WHERE user_id = $1 AND txn_id LIKE 'QRY_%'",
            _TEST_USER_ID,
        )
    finally:
        await conn.close()
...
async def _seed():
    ...
    await set_transaction_categories(_TEST_USER_ID, {
        "QRY_IN_1": "income",
        "QRY_IN_2": "income",
        "QRY_OUT_1": "transport",
...
async def test_income_for_month_sums_per_currency():
    ...
    result = await income_for_month(_TEST_USER_ID, "2026-04")
...
async def test_income_for_month_empty_on_no_data():
    if not _has_db():
        pytest.skip("POSTGRES_DSN not set")
    from orchestrator.app.finance_queries import income_for_month
    result = await income_for_month(_TEST_USER_ID, "2026-03")
    assert result == {}
...
async def test_spending_by_category_ignores_incomes_and_null():
    ...
    result = await spending_by_category(_TEST_USER_ID, "2026-04")
...
async def test_current_balance_per_currency():
    ...
    result = await current_balance(_TEST_USER_ID)
...
async def test_runway_computes_days_from_avg_burn():
    ...
    result = await runway(_TEST_USER_ID, avg_window_days=365)  # wide window so seeded rows count
    usd = result.get("USD")
...
async def test_runway_days_none_when_no_spending():
    ...
    await set_transaction_categories(_TEST_USER_ID, {"QRY_IN_ONLY": "income"})
    from orchestrator.app.finance_queries import runway
    result = await runway(_TEST_USER_ID, avg_window_days=30)
