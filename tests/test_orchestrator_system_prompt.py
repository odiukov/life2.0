...
def test_system_prompt_mentions_six_peer_agents():
    # new contract: orchestrator now has eight peer agents (sleep, workout, nutrition,
    # body, mood, habits, recovery, medication) per persona/isolation refactor
    prompt = _get_system_prompt()
    assert "eight" in prompt.lower() or "8" in prompt
