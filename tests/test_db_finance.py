...
async def _cleanup():
    ...
    try:
        await conn.execute("DELETE FROM finance_transactions WHERE txn_id LIKE 'TEST_%'")
        await conn.execute("DELETE FROM finance_category_cache WHERE desc_key LIKE 'test_%'")
    finally:
        await conn.close()
    yield
    conn = await asyncpg.connect(os.environ["POSTGRES_DSN"])
...
async def test_upsert_finance_rows_is_idempotent():
    ...
    inserted1, skipped1 = await upsert_finance_rows(_sample_rows())
    assert inserted1 == 2 and skipped1 == 0

    inserted2, skipped2 = await upsert_finance_rows(_sample_rows())
    assert inserted2 == 0 and skipped2 == 2

    ids = await fetch_uncategorized_ids()
...
async def test_set_transaction_categories_updates_rows():
    ...
    await upsert_finance_rows(_sample_rows())
    ids = await fetch_uncategorized_ids()
    updates = {tid: ("income" if tid.endswith("A") else "transport")
               for tid in ids if tid.startswith("TEST_")}
    await set_transaction_categories(updates)
    after = await fetch_uncategorized_ids()
...
async def test_category_cache_upsert_and_get():
    ...
    await upsert_category_cache({"test_key_1": "food", "test_key_2": "housing"})
    hits = await get_category_cache(["test_key_1", "test_key_2", "test_missing"])
