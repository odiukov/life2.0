...
def _is_disabled(name: str) -> bool:
    """Mirror the OTEL CLI auto-instrumentation env: opt-out by name.

    `OTEL_PYTHON_DISABLED_INSTRUMENTATIONS` is normally honored only by the
    `opentelemetry-instrument` launcher. We call instrumentors manually here,
    so the env var would otherwise be silently ignored — wire it up explicitly.
    """
    raw = os.environ.get("OTEL_PYTHON_DISABLED_INSTRUMENTATIONS", "")
    return name in {n.strip() for n in raw.split(",") if n.strip()}


def _instrument_frameworks() -> None:
    """Lazy-import each instrumentor so missing deps don't break services that
    lack them."""
    if not _is_disabled("httpx"):
        try:
